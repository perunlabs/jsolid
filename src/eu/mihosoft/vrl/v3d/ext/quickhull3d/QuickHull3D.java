...
class QuickHull3D {
   ...
   * default), then the tolerance will be computed automatically from the point
   * data.
   *
   * @param tol
   *          explicit tolerance
   ...
   * printed first (each preceding by the letter <code>v</code>), followed by
   * the vertex indices for each face (each preceded by the letter
   * <code>f</code>).
   *
   * <p>
  ...
  protected void addNewFaces(
      FaceList newFaces, Vertex eyeVtx, Vector<HalfEdge> horizon) {
    newFaces.clear();
   ...
   * are performed using the distance tolerance <i>tol</i>. Faces are considered
   * non-convex if any edge is non-convex, and an edge is non-convex if the
   * centroid of either adjoining face is more than <i>tol</i> above the plane
   * of the other face. Similarly, a point is considered outside a face if its
